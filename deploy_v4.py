...
print("Iniciando modelagem para Score Aguas do Brasil, Melhor devedor e Melhor CPC")
print("    ")
...
# Carregando o arquivo do modelo CPC
arquivo_cpc = 'modelo_v1_cpc.sav'
Model_cpc = pickle.load(open(arquivo_cpc, 'rb'))
print("Modelo CPC Carregado!")
   
with open('NameFeature_v1_cpc','rb') as arquivo_cpc:
    nome_features_cpc = pickle.load(arquivo_cpc)
...
TODAY = date.today()
YESTERDAY = date.today() - timedelta(days=999)
...
features_cpc = mailing[[ 'QTDE_TITULOS', 'VALOR_DIVIDA_ATIVA' , 'ORIGEM', 'TIPO']] 
...
# Aplicação do modelo treinado na nova base CPC
mailing['SCORE_CPC'] =  Model_cpc.predict_proba(features_cpc)[:,1]
mailing['PONTUACAO_CPC'] = ((mailing['SCORE_CPC'])*10).round()
mailing['DATA_IMPORTACAO'] = TODAY

mailing['BEST_SCORE'] = mailing['SCORE_DEVEDOR'] + (mailing['SCORE_CPC'])
...
auxiliar_2 = []
for x in list(mailing['BEST_SCORE']):
    if 0 <= x < q1:
        auxiliar_2.append('Improvavel')
    elif q1 <= x < q2:
        auxiliar_2.append('Pouco Provavel')
    elif q2 <= x < q3:
        auxiliar_2.append('Provavel')
    elif x >= q3:
        auxiliar_2.append('Muito Provavel')

mailing['CLASSIFICACAO_BEST_SCORE'] = auxiliar_2


q1 = mailing.SCORE_CPC.quantile(0.25)
q2 = mailing.SCORE_CPC.quantile(0.50)
q3 = mailing.SCORE_CPC.quantile(0.75)

auxiliar_1 = []
for x in list(mailing['SCORE_CPC']):
    if 0 <= x < q1:
        auxiliar_1.append('Improvavel')
    elif q1 <= x < q2:
        auxiliar_1.append('Pouco Provavel')
    elif q2 <= x < q3:
        auxiliar_1.append('Provavel')
    elif x >= q3:
        auxiliar_1.append('Muito Provavel')

mailing['CLASSIFICACAO_CPC'] = auxiliar_1 

q1 = mailing.SCORE_DEVEDOR.quantile(0.25)
q2 = mailing.SCORE_DEVEDOR.quantile(0.50)
q3 = mailing.SCORE_DEVEDOR.quantile(0.75)
